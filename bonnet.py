...
while True:
    ...
    # Change text based on button input
    if not button_a.value:
        draw.text((10, 90), "Button A is pressed!", font=font, fill=0)
    elif not button_b.value:
        draw.text((10, 90), "Button B is pressed!", font=font, fill=0)

    # Change background color based on joystick X input
    background_color = int((x_value / 2048) * 255)
    draw.rectangle((0, 0, oled.width, oled.height), outline=0, fill=background_color)

    # Adjust brightness based on joystick Y input
    brightness = int((y_value / 2048) * 255)
